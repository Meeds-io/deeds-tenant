...
public class TenantManagerService implements Startable {
  ...
  protected TenantServiceFacade tenantServiceSkeleton;
  ...
  public boolean isTenantManager(String address) {
    if (isTenant() && StringUtils.isNotBlank(address)) {
      ...
      if (deedTenantHost == null || StringUtils.isBlank(deedTenantHost.getManagerAddress())) {
        boolean isTenantManager = getTenantServiceSkeleton().isTenantManager(address, getNftId());
  ...
  private DeedTenantHost retrieveDeedTenant() {// NOSONAR
    return getTenantServiceSkeleton().getDeedTenant(getNftId());
  }
  ...
  private TenantServiceFacade getTenantServiceSkeleton() {
    if (tenantServiceSkeleton == null) {
      try {
        tenantServiceSkeleton = SpringContext.getSpringBean(TenantServiceFacade.class);
      } catch (Exception e) {
