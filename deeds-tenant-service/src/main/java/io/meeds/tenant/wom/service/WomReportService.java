...
import io.meeds.tenant.wom.storage.HubReportStorage;

@Service
public class WomReportService {

  private static final Log     LOG                         = ExoLogger.getLogger(WomReportService.class);
  ...
  public HubReportLocalStatus sendReport(RewardReport rewardReport) throws WomException {
    ...
    if (!rewardReport.isCompletelyProceeded()) {
      ...
      String signature = womService.signHubMessage(reportData);
  ...
  private HubReportLocalStatus refreshReportFromWoM(long periodId) throws WomException {
    ...
    if (report == null || !StringUtils.equalsIgnoreCase(report.getHubAddress(), womService.getHubAddress())) {
  ...
  public HubReportPayload toReport(RewardReport rewardReport) {
    ...
    return toHubReport(rewardReport,
                       womService.getHubAddress(),
                       womService.getDeedId(),
                       womService.computeUsersCount(),
  ...
  private boolean isValidRewardDate(RewardPeriod rewardPeriod) {
    ...
    return SEND_OUTDATED_REPORT || womService.getHubJoinDate().isBefore(endDate);
  }
  ...
  private HubReportStatusType computeReportStatusType(RewardPeriod rewardPeriod, String status) {
    if (StringUtils.isBlank(status)) {
      if (!womService.isDeedHub() || isValidRewardDate(rewardPeriod)) {
